...
def fixsender(sender, all_senders=None):
    global dns_mapping
    global mapping
...
def parsemaildate(md):
    ...
    try:
        tz = pieces[4]
        # Only want numeric timezone values
        ival = int(tz)
        if tz == '-0000':
            tz = '+0000'
        tzh = tz[:3]
...
# Parse out the info...
def parseheader(hdr, all_senders=None):
    ...
    sender = fixsender(sender, all_senders)
...
for message_row in cur_1:
    dns_mapping[message_row[0].strip().lower()] = message_row[1].strip().lower()
...
print('Loaded all_senders', len(all_senders), 'and mapping', len(mapping), 'dns mapping', len(dns_mapping))
...
for message_row in cur_1:
    hdr = message_row[0]
    parsed = parseheader(hdr, all_senders)
    ...
    cur.execute('''
        INSERT OR IGNORE INTO Messages (guid, sender_id, subject_id, sent_at, headers, body)
        VALUES (?, ?, ?, datetime(?), ?, ?)''',
